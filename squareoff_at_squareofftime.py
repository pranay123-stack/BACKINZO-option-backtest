...
def squareoff_all_legs_direct(csv_file_path,underlying_symbol, entry_time, squareoff_time, legs):
    df = pd.read_csv(csv_file_path)
    ...
    for leg in legs:
            ...
            squareoff_all.append({'id': leg_id, 'leg': leg, 'entry_order_details': entry_order_details})
...
result = squareoff_all_legs_direct(backtest_config['entry_csv_file_path'],backtest_config['Underlying_Symbol'],backtest_config['entry_time'],backtest_config['squareoff_time'], backtest_config['entrylegs_details'])
print(result)
