...
def take_reentry(df, trades_df, entry_symbol, pending_legs, leg, exit_time, reentry_entry_time, wait_time, entry_price):
    
    data_found = False

    while leg['count'] > 0:
        # Wait until the cooldown period has passed before taking reentry
        # Assuming 'exit_time' and 'Time' in df are pandas datetime objects for accurate time comparison
          # Ensure times are datetime objects
      
        cool_down_time = pd.to_datetime(exit_time, format='%H:%M:%S') + pd.Timedelta(minutes=wait_time)  # Adjust timedelta as per your time units
        print("cool down time",cool_down_time)
        cool_down_entries = df[(pd.to_datetime(df['Time'], format='%H:%M:%S') > cool_down_time ) &(pd.to_datetime(df['Time'], format='%H:%M:%S') < squareoff_time) &  (df['Ticker'] == entry_symbol)]
        
        if cool_down_entries.empty:
            break  # If no entries after cooldown, break the loop

        for index, row in cool_down_entries.iterrows():
            # Check if the entry price has cooled down compared to some logic, here simple example
            if row['Low'] < entry_price: 
                data_found = True
                
              
                entry_order_details = reentry_place_order_at_atm_price(df, leg ,row['High'], row['Time'],reentry_entry_time)
                ...
                if exit_row is not None:
                    ...
                    leg['count'] -= 1
                    break  # Break after re-entry is taken
                else:
                    ...
                    break  # Break if no valid exit found after re-entry
        if data_found == False:
           break
...
def place_order_at_atm_price(df, leg, entry_time):
    ...
    symbol_to_search = f"{leg['Symbol']}{strike_price_row_at_time_t(df, atm_row, entry_time)}{leg['type']}.NFO"
    ticker_row_entry = ticker_row_at_time(df, symbol_to_search, entry_time)
    entry_price = ohlc_row_at_time_t(df,ticker_row_entry)
    target_price = entry_price * 1.1 if entry_price else None
...
def reentry_place_order_at_atm_price(df, leg ,entry_price_row,exit_time, entry_time):
    atm_row = atm_row_at_time_t(df, entry_time)
    symbol_to_search = f"{leg['Symbol']}{strike_price_row_at_time_t(df, atm_row, entry_time)}{leg['type']}.NFO"
    ticker_row_entry = ticker_row_at_time(df, symbol_to_search, entry_time)
    entry_price = ohlc_row_at_time_t(df,ticker_row_entry)
...
def straddle_backtest(csv_file_path, entry_time,squareoff_time, legs):
    ...
    for leg in legs:
        ...
        if exit_row is not None and not exit_row.empty: 
            ...
            trades_df, pending_legs = take_reentry(df, trades_df, symbol_to_search, pending_legs, leg,exit_time,reentry_entry_time,wait_time,entry_order_details['entry_price'])
...
legs = [
    {'Symbol':'BANKNIFTY06MAR24','type': 'CE','EntryType':"Sell",'ExitType':"Buy" ,'lot_size': 15,'count':1 },   
    {'Symbol':'BANKNIFTY06MAR24','type': 'PE','EntryType':"Sell",'ExitType':"Buy", 'lot_size': 15,'count':1 },  
